...
ORGAN_DATASET_DIR = '/pub/data/yangdeq/Flare2024/'
ORGAN_LIST = '/home/yangdq/project/module/CLIP-Driven-Universal-Model_FLARE2024/dataset/FLARE2024_list/Flare_train.txt'
NUM_WORKER = 13
# NUM_CLASS = 32
NUM_CLASS = 51
## full list
...
class ToTemplatelabeld(MapTransform):
    ...
    def __call__(self, data: Mapping[Hashable, NdarrayOrTensor]) -> Dict[Hashable, NdarrayOrTensor]:
        ...
        # print(f"label unique {np.unique(d['label'])} in {d['name']}\n")
        return d
...
def generate_label(input_lbl, num_classes, name, TEMPLATE, raw_lbl):
    ...
    for b in range(B):
        ...
        print(f"########before unique result {np.unique(result[b])} in {name[b]}")
        ...
        print(f"########after unique result {np.unique(result[b])} in {name[b]}")
...
for index, batch in enumerate(train_loader):
    ...
    print( "after",name,np.unique(store_y))

    if not os.path.exists(post_dir):
        os.makedirs(post_dir)
    if not os.path.exists(ORGAN_DATASET_DIR + name + '.h5'):
