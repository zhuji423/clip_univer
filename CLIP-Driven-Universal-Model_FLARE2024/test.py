...
def validation(model, ValLoader, val_transforms, args):
    save_dir = '/pub/data/yangdeq/CLIP/data/amos/out/' + args.log_name + f'/test_healthp_{args.epoch}'
    ...
    with open('/pub/data/yangdeq/CLIP/data/amos/out/'+args.log_name+f'/test_{args.epoch}.txt', 'w') as f:
        for key in TEMPLATE.keys():
...
def main():
    ...
    ## model load
    parser.add_argument('--resume', default='/pub/data/yangdeq/CLIP/data/amos/out/unet', help='The path resume from checkpoint')
    ...
    parser.add_argument('--data_root_path', default='/pub/data/yangdeq/CLIP/data/amos/', help='data root path')
    parser.add_argument('--data_txt_path', default='/home/yangdq/project/module/CLIP-Driven-Universal-Model/dataset/amos_data_list/', help='data txt path')
    ...
    model_for_val = [pth for pth in os.listdir(args.resume) if "pth" in pth]
    checkpoint = torch.load(os.path.join(args.resume,model_for_val[0])) 
