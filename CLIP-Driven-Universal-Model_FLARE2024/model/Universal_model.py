...
class Universal_model(nn.Module):
    ...
    def forward(self, x_in):
        ...
        # task_encoding torch.Size([34, 256, 1, 1, 1])
        x_feat = self.GAP(dec4) ##[4, 256, 1, 1, 1]
        b = x_feat.shape[0]
        logits_array = []
        for i in range(b):
            x_cond = torch.cat([x_feat[i].unsqueeze(0).repeat(self.class_num,1,1,1,1), task_encoding], 1) ## 都变化为[34,256,1,1,1]
            params = self.controller(x_cond) ## torch.Size([34, 153, 1, 1, 1])
